...
export default{
    ...
    methods: {
        ...
        fileUpload(event){
            // 여러개의 이미지일 경우 for문을 실행하여 productImage에 리스트 형태로 push
            this.productImage = event.target.files[0];
        },
        async productCreate(){
            try {
                // multi-part-formdata 또는 url인코딩 방식인 경우, FormData 클래스 사용
                const data = new FormData();
                data.append("name", this.name);
                data.append("category", this.category);
                data.append("price", this.price);
                data.append("stockQuantity", this.stockQuantity);
                // image를 아예 append시키지 않으면, 백엔드 dto에 null로 매핑되나, null을 append 시키면 백엔드에서 타입에러발생
