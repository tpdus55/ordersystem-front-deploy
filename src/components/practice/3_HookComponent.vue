<template>
    <div>hook함수 테스트</div>
    <button @click="moveScreen()">다른화면으로 이동(라우트 방식)</button>
    <div>{{ count1 }}</div>
    <button @click="increaseCount()">count1값 증가</button>
...
export default{
    ...
    // 컴포넌트가 화면상에서 제거(이동)될때 실행되는 함수
    // 예시) 채팅(알림)화면을 벗어나 다른화면으로 이동시에 실시간 연결 종료 요청(물리적 화면을 종료시키는 경우는 제외)
    beforeUnmount(){
    ...
    // 종속된 데이터가 변경될때 함수를 실행하여 값을 재계산해서 리턴하는 함수
    // 장바구니 수량 보여줄때
    computed:{
